...
function getInputFiles(evt) {
    var files = isInput(evt.target)
        ? evt.target.files
            ...
            : []
        : [];
    return files.map(function (file) { return file_1.toFileWithPath(file); });
}
...
function fromDataTransferItem(item) {
    ...
    var fwp = file_1.toFileWithPath(file);
...
function createEmptyDirFile(entry) {
    var file = new File([], entry.name);
    var fwp = file_1.toFileWithPath(file, entry.fullPath + '/');
    return fwp;
}
